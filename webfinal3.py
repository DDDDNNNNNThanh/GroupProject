...
if selected == "About Us":
    ...
    with st.container():
        ...
        with col1:
            st.markdown("""<p style='color: #244939; text-align: center; font-family: Candara;'>
    ...
    if (signinbutton or st.session_state.signin) and signupbutton == False:
        ...
        if st.sidebar.button('sign in') or st.session_state.signinsubmit == True:
            ...
            if not check_signin:
                ...
                del currentdata['ID']
                del currentdata['Food Code']
                currentdata.index += 1
                col3_1, col3_2 = st.columns([5,1])
                with col3_1:
                    # new: color
                    st.table(currentdata.style.apply(ft.color_coding, axis=1))
                with col3_2:
                    # new: remove all expired food
                    if st.button("double-click to remove all expired food"):
                ...
                with st.expander("Calo Visualization"):
                    ft.visualization(currentid)
    ...
    st.title('No Food No Life :sunglasses: :pineapple:')
